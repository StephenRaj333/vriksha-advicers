...
function resizeHeaderOnScroll() {
  const distanceY = window.scrollY || document.documentElement.scrollTop,
    headerEl = document.getElementById('logo_resizer');

  // Set shrinkOn value based on window size
  const shrinkOn = window.innerWidth < 600 ? 50 : 300;

  if (distanceY > shrinkOn) {
    headerEl.classList.add("bigger");
    headerEl.classList.remove("logo");
    headerEl.classList.add("transition"); // Add a class for transition effect
  } else {
    headerEl.classList.remove("bigger");
    headerEl.classList.add("logo");
    headerEl.classList.remove("transition"); // Remove transition class when not needed
  }   
} 
